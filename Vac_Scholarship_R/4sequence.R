...
S4 <- read.phyDat("test.fa",format="fasta", type="DNA")
#read in the fsc tree (only 1 )
T4 <-read.nexus("4seq_1_true_trees.trees")
T4 <- as.phylo(T4)
T4
...
T4 <- reorder(T4, "postorder")
#check postorder
T4$edge

#rearranges S4 so that the order of the sequences matches the tree
S4 <- S4[order(T4$tip.label)]
...
t[1]<- t[2]
  ...
  for(i in 1:ne){
    if(Edges[i,2]<=length(S4)){
      for(j in 1:ns){
        tmp <- matrix( c(0,0,0,0), nrow=4, ncol=1)      # creates empty column matrix 
        tmp[S4[[Edges[i,2]]][j],1] <- 1                             # Inputs 1 into column where there is the nucleotide
...
Likelihood4(c(0.4,1,0.5,1.5,0.25,1.75))
...
output<- function(c){ 
out <- optim(par = c, fn=Likelihood4, lower=0.00000001,method="L-BFGS-B")
out$par[1]<- out$par[2]
return(out)
}

output(c(0,1,1,1.5,0,1.75))
